...
export function StarOutline({ size = 28, color = '#c4b5fd', strokeWidth = 2.4, style }: {
...
export function HeartFill({ size = 22, color = '#fbcfe8', style }: {
...
export function Squiggle({ width = 60, color = '#a78bfa', strokeWidth = 3, style }: {
  width?: number; color?: string; strokeWidth?: number; style?: React.CSSProperties
}) {
  return (
    <svg width={width} height={width / 4} viewBox="0 0 60 15" fill="none" style={style}>
      <path
...
export function Zigzag({ width = 50, color = '#f0abfc', strokeWidth = 3, style }: {
  width?: number; color?: string; strokeWidth?: number; style?: React.CSSProperties
}) {
  return (
    <svg width={width} height={width / 4} viewBox="0 0 50 12" fill="none" style={style}>
      <path
...
export function DotsCluster({ size = 50, color = '#bae6fd', style }: {
  size?: number; color?: string; style?: React.CSSProperties
}) {
  // 3x3 不规则点阵
  const dots = [
